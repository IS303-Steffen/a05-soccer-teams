...
def test_02_printed_messages(current_test_name, input_test_cases):
    try:
        ...
        for input_test_case in input_test_cases:
            ...
            # replace any number with <number> since the numbers are randomized.
            normalized_captured_print_statements_list = [re.sub(r'\d+', '<number>', print_statement) for print_statement in normalized_captured_print_statements_list]
            ...
            for expected_phrase in expected_printed_messages:
                ...
                expected_phrase = re.sub(r'\d+', '<number>', expected_phrase) # replace any number with <number>
                normalized_captured_print_statements_str = re.sub(r'\d+', '<number>', normalized_captured_print_statements_str)
